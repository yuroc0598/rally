...
def _box_iou(left: Sequence[float], right: Sequence[float]) -> float:
    ...
    union = (lx1 - lx0) * (ly1 - ly0) + (rx1 - rx0) * (ry1 - ry0) - overlap
...
def _pose_record(
    ...
    for hand, joint in (("left", 9), ("right", 10)):
        if float(confidence[joint]) > 0.2:
            ...
            # Image y grows downward.  Positive values therefore mean that the wrist is
            # above the shoulder, expressed in player-body lengths.  Preserve each hand
            # separately: a service sequence must follow one physical arm through load,
            # overhead acceleration and follow-through rather than mixing the toss arm
            # from one frame with the racket arm from another.
            wrist_overhead[hand] = float(-local[1])
    if not wrists:
        return None
    return {
        "time": float(time_s),
        ...
        "keypoints_norm": [
            [float(point[0] / width), float(point[1] / height)] for point in pose[:17]
        ],
        "keypoint_confidence": [float(value) for value in confidence[:17]],
        "wrists_body": wrists,
        "wrist_overhead_by_hand": wrist_overhead,
...
def _racket_evidence(
    ...
    return {
        ...
        "racket_wrist_distance_body": (round(float(distance), 4) if distance is not None else None),
    }


def _smoothed_records(records: Sequence[dict], radius_s: float) -> list[dict[str, Any]]:
...
def _action_candidates(
    ...
    for index, current in enumerate(records):
        time_s = float(current["time"])
        if any(abs(time_s - serve) <= cfg.pose_service_attempt_mask_s for serve in ordered_serves):
            continue
        pre = [
            record
        ...
        for hand in ("left", "right"):
            ...
            reversal = bool(
                preparation_cosine <= -0.10 or preparation_vector[0] * forward_vector[0] <= -0.025
            )
            compact_zone = (
                abs(float(current["court_y_m"]) - NET_Y) <= cfg.pose_compact_stroke_net_distance_m
            )
            ...
            groundstroke = bool(
                ...
                and forward >= 0.16
                and follow >= cfg.pose_action_min_follow_body
            ...
            compact = bool(
                compact_zone
                and speed >= 0.85 * cfg.pose_action_min_speed_body_s
                and backswing >= 0.12
                and forward >= 0.14
                and follow >= 0.14
                and through >= 0.30
                and elbow >= 105.0
                and extension_gain >= 4.0
                and reversal
                and follow_cosine >= -0.15
            )
            overhead_action = bool(
                overhead
                and speed >= 0.90 * cfg.pose_action_min_speed_body_s
                and backswing >= 0.12
                and forward >= 0.14
                and follow >= cfg.pose_action_min_follow_body
                and through >= 0.30
                and reversal
                and follow_cosine >= -0.20
            )
            action_type = (
                "overhead_stroke_proxy"
                if overhead_action
                else "compact_stroke_proxy"
                if compact
                else "groundstroke_proxy"
                if groundstroke
                else None
            )
            ...
            if score >= cfg.pose_action_min_confidence:
                classified.append(
                    (
                        score,
                        backswing + forward + follow + through,
                        action_type,
                        hand,
                        backswing,
                        forward,
        ...
        (
            ...
            reversal,
            compact_zone,
        ) = max(classified, key=lambda item: (item[0], item[1]))
        output.append(
            {
                "time": round(time_s, 3),
                "actor_id": current["actor_id"],
                "team_id": current.get("team_id"),
                "actor_end": current["actor_end"],
                "action": action_type,
                "hand": hand,
                ...
                "compact_motion_zone": compact_zone,
                ...
                "evidence": [
                    "ordered_wrist_preparation",
                    "direction_change_into_forward_swing",
                    "local_forward_acceleration",
                    "post_contact_follow_through",
                    "target_court_player_pose",
                    (
                        "detected_racket_near_wrist"
                        if racket_associated
    ...
    for candidate in sorted(output, key=lambda item: float(item["confidence"]), reverse=True):
        if any(
            candidate["actor_id"] == prior["actor_id"]
            and abs(float(candidate["time"]) - float(prior["time"])) < cfg.pose_action_nms_s
...
def _stroke_episodes(proposals: Sequence[dict[str, Any]], cfg) -> list[dict[str, Any]]:
    ...
    for actor_proposals in by_actor.values():
        ...
        for proposal in sorted(actor_proposals, key=lambda item: float(item["time"])):
            if (
                not clusters
                or float(proposal["time"]) - float(clusters[-1][-1]["time"])
