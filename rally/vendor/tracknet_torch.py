"""Vendored, unmodified BallTrackerNet (yastrebksv/TennisProject port of TrackNetV2).

Do NOT "fix" the architecture: the ConvBlock order (Conv -> ReLU -> BatchNorm2d), the
256-channel per-pixel output, the nearest-neighbour Upsample, and the lack of U-Net skip
connections are all fixed by the pretrained checkpoint (models/tracknet_yastrebksv.pt).
