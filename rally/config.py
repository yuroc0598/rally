...
class RallyConfig:
    ...
    # A cut starts at the measured beginning of the server's stable baseline setup,
    # never at a fixed offset from the overhead peak.  The fallback is used only when
    # the track begins too late to observe setup onset.
    pose_serve_setup_lookback_s: float = 12.0
    ...
    # Brief pose dropouts may be bridged, but a sustained relaxed interval is a hard
    # state boundary.  Long two-sided live-state bouts can recover a missed serve and
    # missed racket contact anywhere in the video.
    pose_live_gap_bridge_s: float = 1.75
    pose_live_reset_break_s: float = 0.75
    ...
    pose_between_min_fraction: float = 0.45
    ...
    # A serve can start a valid point even when the small/far returner's racket pose is
    # missed. Sustained two-sided ready posture is independent evidence that play
    # continued; wrist gestures or walking alone do not satisfy this gate.
    pose_engaged_min_s: float = 0.80
    pose_engaged_min_fraction: float = 0.50
    pose_engaged_min_ready_fraction: float = 0.25
    pose_engaged_search_s: float = 5.0
    pose_endpoint_response_s: float = 3.25
    pose_endpoint_max_unexplained_tail_s: float = 4.0
    pose_unreturned_transition_search_s: float = 5.0
    pose_partial_start_max_s: float = 5.0
    ...
    court_weights: str = field(default_factory=lambda: _model_from_env(
        "RALLY_COURT_WEIGHTS", DEFAULT_COURT_MODEL))
    # ---- non-play exclusion -------------------------------------------------
    ...
    def __post_init__(self) -> None:
        ...
        for name in (
            "player_fps", "pose_timeline_fps", "pose_refine_fps",
            "min_rally_s",
        ):
            if getattr(self, name) <= 0:
                raise ValueError(f"{name} must be positive")
        for name in (
            "pose_refine_pre_s", "pose_refine_post_s", "pose_track_box_max_gap_s",
            "pose_refine_motion_windows_per_minute",
            "pose_serve_setup_lookback_s", "pose_serve_setup_max_gap_s",
            "pose_serve_setup_fallback_s",
            "pose_action_pre_s", "pose_action_post_s", "pose_action_smoothing_s",
            "pose_action_nms_s", "pose_stroke_episode_gap_s",
            "pose_service_attempt_mask_s", "pose_service_retry_max_gap_s",
            "pose_service_retry_state_max_gap_s",
            "pose_service_impossible_end_switch_s",
            "pose_first_return_max_s", "pose_exchange_max_gap_s",
            "pose_live_gap_bridge_s", "pose_live_reset_break_s",
            "pose_live_candidate_min_s", "pose_exchange_start_lookback_s",
            "pose_point_reset_delay_s", "pose_between_min_s",
            "pose_between_max_median_wrist_speed_body_s",
            "pose_engaged_min_s", "pose_engaged_search_s",
            "pose_endpoint_response_s", "pose_unreturned_transition_search_s",
            "pose_endpoint_max_unexplained_tail_s",
            "pose_partial_start_max_s",
        ...
        if self.pose_serve_min_wrist_rise < 0 or self.pose_serve_leg_drive_deg < 0:
        ...
        for name in (
            "pose_action_min_backswing_body", "pose_action_min_follow_body",
            "pose_action_min_through_body", "pose_compact_stroke_net_distance_m",
        ):
            if getattr(self, name) < 0:
                raise ValueError(f"{name} must be non-negative")
        ...
        for name in (
            "pose_serve_min_score", "pose_serve_unreturned_min_score",
            "pose_between_min_fraction", "pose_between_max_ready_fraction",
            "pose_engaged_min_fraction", "pose_engaged_min_ready_fraction",
